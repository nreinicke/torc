...
pub fn batch_complete_jobs(
    ...
    id: i64,
    batch_complete_jobs_request: models::BatchCompleteJobsRequest,
) -> Result<models::BatchCompleteJobsResponse, Error<BatchCompleteJobsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
    let p_body_batch_complete_jobs_request = batch_complete_jobs_request;

    let uri_str = format!(
        "{}/workflows/{id}/batch_complete_jobs",
    ...
    req_builder = req_builder.json(&p_body_batch_complete_jobs_request);
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::BatchCompleteJobsResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::BatchCompleteJobsResponse`"
                ))));
            }
        }
    } else {
        let content = resp.text()?;
        let entity: Option<BatchCompleteJobsError> = serde_json::from_str(&content).ok();
